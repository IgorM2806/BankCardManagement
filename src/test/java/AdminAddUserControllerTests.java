...
public class AdminAddUserControllerTests {
    ...
    void testAdminAddUserController_Success() throws Exception {
        var userDto = new CreateUserDTO("Tester1", "Testerov1",
                "Testerovich1", RoleEnum.ADMIN, "temp");

        var jsonContent = objectMapper.writeValueAsString(userDto);

        Users createdUser = new Users();
        when(adminService.createUser(anyString(), anyString(), anyString(), any(RoleEnum.class), anyString()))
    ...
    void testAdminAddUserController_EmptyFields() throws Exception {
        ...
        CreateUserDTO userDto = new CreateUserDTO(firstName, lastName, patronymic, role, passwordHash);
    ...
    void testAdminAddUserController_DuplicateUser() throws Exception {
        ...
        doThrow(exception)
                .when(adminService)
                .createUser(firstName, lastName, patronymic, role, passwordHash);
        ...
        verify(adminService).createUser(firstName, lastName, patronymic, role, passwordHash);
    }
}
